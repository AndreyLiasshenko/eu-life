let add_comm = document.getElementsByClassName("add_comment");
let new_comm_body = document.getElementsByClassName("new_comment");
let arr_comm_body = Array.from(new_comm_body);
let arr_button = Array.from(add_comm);

//showing all comment body
...
for (let i = 0; i< arr_button.length; i++) {
    arr_button[i].addEventListener("click", function () {
        ...
        arr_comm_body[i].style.display = "block";
    })
}
